...
export default function BreadcrumbsWithIcon({Parent,Child,List}) {
  ...
  return (
        ...
        <div className={`rounded-md shadow-sm mx-4 grid grid-cols-1  justify-center items-center gap-3 bg-cover ${LightModeState==LightMode().type?"bg-whiteTheme_T2 tc-whiteTheme_T1":"bg-darkTheme_T2 tc-darkTheme_T1"}`}>
            
            {List==true?<div className=" col-span-1 w-full flex flex-col justify-center items-center">
     ...
     <Breadcrumbs separator={<p className={`${LightModeState==LightMode().type?"tc-whiteTheme_T1":"tc-darkTheme_T1"}`}>/</p>} fullWidt={true} className={`bg-inherit   ml-4  col-span-1  ${LightModeState==LightMode().type?"tc-whiteTheme_T1":"tc-darkTheme_T1"}`}  >
