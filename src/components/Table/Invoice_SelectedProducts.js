...
import React from "react";
  ...
  export default function Invoice_SelectedProducts({ProductsData,SetProductsData}) {
    ...
    return (
      ...
      <Card className={`w-full  h-full  shadow-none bg-inherit ${LightModeState==LightMode().type?"tc-whiteTheme_T1 ":"tc-darkTheme_T1 "}`}>
      <CardHeader floated={false} shadow={false} className={`rounded-none bg-transparent ${LightModeState==LightMode().type?"tc-whiteTheme_T1 ":"tc-darkTheme_T1 "}`}>
                  ...
                  return (
                    <tr key={index}>
                      ...
                      <Tooltip content="Delete Product">
                        <IconButton variant="text" onClick={()=>{SelectedRow.current=ProductCode;SetOpenDeleteDialog(true)}}>
        ...
        <ConfirmDeleteDialog  Open={OpenDeleteDialog} Action={()=>{HandleDeleteRow(SelectedRow.current)}} HandleOpen={()=>{SetOpenDeleteDialog(!OpenDeleteDialog)}} Icon={'<i class="fa-solid fa-trash h-5 w-5 mx-1"></i>'} Title={"Delete Product"} Content="Are you sure you want to delete this product?" />
        <InvoiceProductDialog SetAllData={SetProductsData}   Open={OpenInvoiceAddProduct} Action={()=>{}} HandleOpen={()=>{SetOpenInvoiceAddProduct(!OpenInvoiceAddProduct)}} Icon={'<i class="fa-solid fa-gear"></i>'} Title={"Add Product to Invoice"} Content="" />
        </Card>
    );
  }