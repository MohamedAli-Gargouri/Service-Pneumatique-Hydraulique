...
import React from "react";
  ...
  export default function Invoice_ProductsList({P_Selected, P_Quantities, P_Products}) {
    ...
    const [Quantities,setQuantities]=React.useState(TABLE_ROWS.map((item)=>{ return({ProductCode:item.ProductCode,Quantity:1})}))
                ...
                ({ ProductCode,img,Brand,Model,Price}, index) => {
                  ...
                  return (
                    <tr key={ProductCode}>
                        <td className={classes}>
                          {
                            ...
                            <Checkbox
                                defaultChecked={VerifySelect(ProductCode)}
                                  onClick={(e)=>{HandleSelectChange(e,ProductCode)}}
                                  ripple={false}
                      ...
                      <td className={classes}>
                        <QuantityInput QuantityLabel={"Quantity"} IdLabel={"ProductCode"} Id={ProductCode} Quantities={Quantities} setQuantities={setQuantities}  />
                      </td>
