...
export function readBinaryData(file: any) {
...
/**
 * This function gets the file name from a bolb URL
 * @param blobUrl the blob file URL
 * @returns
 */
export function getFileNameFromBlobUrl(blobUrl: any) {
...
export function getFileExtensionFromArrayBuffer(arrayBuffer: any) {
  // Define a mapping of common file type signatures and their corresponding extensions
  const fileSignatures: any = {
    '89504E47': 'png', // PNG
    '47494638': 'gif', // GIF
    '25504446': 'pdf', // PDF
  ...
  const signatureBytes: any = Array.from({ length: 8 }, (_, index) =>
    dataView.getUint8(index).toString(16).padStart(2, '0'),
  )
    .join('')
    .toUpperCase();
 ...
 * @returns
 */
export function getImageBlobUrl(extension: string, fileBinary: any) {
  return new Promise((resolve, reject) => {
